...
def python(code, show=True):
    ...
    """
    setup = "import os; os.environ[\'DJANGO_SETTINGS_MODULE\']=\'%s.settings\';" % (env.proj_dirname)
    full_code = 'python -c "%s%s"' % (setup, code.replace("`", "\\\`"))
    with project():
        result = run(full_code, show=False)
        if show:
            print_command(code),result
    return result
...
def static():
    ...
    return python("from django.conf import settings;"
                  "print settings.STATIC_ROOT", show=True).split("\n")[-1]
...
def static_url():
    ...
    """
    return python("from django.conf import settings;"
                  "print settings.STATIC_URL", show=True).split("\n")[-1]
